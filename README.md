...
* [`src/tomate_web/styles.cljs`](src/clj/tomate_web/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/tomate_web/`](src/tomate_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/tomate_web/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `tomate-web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tomate_web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tomate-web.example
  (:require [tomate-web.config :as config])
